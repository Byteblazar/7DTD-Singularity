    ...
    public class Singularity : IModApi
    {
        ...
        public void InitMod(Mod _modInstance)
        {
            ...
            ModEvents.PlayerSpawnedInWorld.RegisterHandler((ref ModEvents.SPlayerSpawnedInWorldData data) =>
            {
                ...
                {
                    var original = AccessTools.Method(typeof(GameManager), nameof(GameManager.ItemActionEffectsServer), new Type[] { typeof(int), typeof(int), typeof(int), typeof(int), typeof(Vector3), typeof(Vector3), typeof(int) });
                    var prefix = AccessTools.Method(typeof(GameManager_Patches), nameof(GameManager_Patches.Prefix_ItemActionEffectsServer));
                    harmony?.Patch(
                        original,
                        prefix: new HarmonyMethod(prefix));

                    original = AccessTools.Method(typeof(ItemActionRanged), nameof(ItemActionRanged.ItemActionEffects), new Type[] { typeof(GameManager), typeof(ItemActionData), typeof(int), typeof(Vector3), typeof(Vector3), typeof(int) });
                    prefix = AccessTools.Method(typeof(ItemActionRanged_Patches), nameof(ItemActionRanged_Patches.Prefix_ItemActionEffects));
                    harmony?.Patch(
                        original,
                        prefix: new HarmonyMethod(prefix));
                }
